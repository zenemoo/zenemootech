...
import { contactApi, subscriberApi, authApi, emailApi, userManagementApi, adminNotificationApi, portalApi } from '../services/api';
import { supabase } from '../lib/supabaseClient';

interface AdminDashboardProps {
  onExit: () => void;
  initialTab?: 'users-rbac' | 'admin-notifs' | 'team' | 'partners' | 'opportunities' | 'inquiries' | 'subscribers' | 'history' | 'telemetry' | 'keys' | 'ai-analytics';
...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({ onExit, initialTab, isStandaloneEmailView }) => {
  ...
  const [activeTab, setActiveTab] = useState<
    'users-rbac' | 'admin-notifs' | 'team' | 'partners' | 'opportunities' | 'inquiries' | 'subscribers' | 'history' | 'telemetry' | 'keys' | 'ai-analytics'
  >((initialTab as any) || 'team');
  ...
  // Admin Notification Engine State
  const [adminNotifications, setAdminNotifications] = useState<any[]>([]);
  const [isNotifModalOpen, setIsNotifModalOpen] = useState(false);
  const [newNotifTitle, setNewNotifTitle] = useState('');
  const [newNotifMessage, setNewNotifMessage] = useState('');
  const [newNotifType, setNewNotifType] = useState('info');
  const [newNotifTargetType, setNewNotifTargetType] = useState<'broadcast' | 'individual'>('broadcast');
  ...
  const navItems = [
    { id: 'users-rbac', name: 'User & RBAC Management', icon: ShieldCheck, count: appUsers.length },
    { id: 'admin-notifs', name: 'Notification Dispatcher', icon: Bell, count: adminNotifications.length },
  ...
  return (
    <div className="min-h-screen bg-[#030304] text-slate-200 flex relative z-50 font-sans overflow-hidden w-full">
      ...
      <div className="flex-1 flex flex-col min-w-0 h-screen overflow-y-auto bg-[#030304]">
        ...
        {/* TAB 0-A: USER & RBAC MANAGEMENT PANEL */}
        {activeTab === 'users-rbac' && (
          <div className="space-y-8">
            {/* Top Bar: Stats Metrics & Create User Button */}
            <div className="grid grid-cols-1 md:grid-cols-4 gap-5">
              <div className="modern-dashboard-card p-6 flex items-center justify-between">
                <div className="space-y-1">
                  <span className="text-[10px] font-mono text-slate-400 uppercase tracking-wider block">Total User Accounts</span>
                  <span className="text-3xl font-extrabold text-white block font-mono">{appUsers.length}</span>
                  <span className="text-[10px] font-mono text-cyan-400 block">Active Portal Users</span>
                </div>
              ...
              <div className="modern-dashboard-card p-6 flex items-center justify-between">
                <div className="space-y-1">
                  <span className="text-[10px] font-mono text-slate-400 uppercase tracking-wider block">Administrators</span>
                  <span className="text-3xl font-extrabold text-cyan-300 block font-mono">
                    {appUsers.filter((u) => u.role === 'admin').length}
                  </span>
                  <span className="text-[10px] font-mono text-purple-400 block">Full System Access</span>
                </div>
              ...
              <div className="modern-dashboard-card p-6 flex items-center justify-between">
                <div className="space-y-1">
                  <span className="text-[10px] font-mono text-slate-400 uppercase tracking-wider block">HR Personnel</span>
                  <span className="text-3xl font-extrabold text-purple-300 block font-mono">
                    {appUsers.filter((u) => u.role === 'hr').length}
                  </span>
                  <span className="text-[10px] font-mono text-emerald-400 block">HR Portal Access</span>
                </div>
                <div className="p-3.5 rounded-xl bg-purple-500/10 text-purple-300 border border-purple-500/20">
              ...
              <div className="modern-dashboard-card p-6 flex items-center justify-between">
                <div className="space-y-1">
                  <span className="text-[10px] font-mono text-slate-400 uppercase tracking-wider block">Team Members</span>
                  <span className="text-3xl font-extrabold text-emerald-300 block font-mono">
                    {appUsers.filter((u) => u.role === 'team_member').length}
                  </span>
                  <span className="text-[10px] font-mono text-slate-400 block">Team Portal Access</span>
                </div>
            ...
            {/* Actions Bar */}
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4 glass-panel p-4 rounded-2xl border border-white/10 font-mono text-xs">
              <div className="flex items-center gap-3 w-full sm:w-auto">
                <div className="relative flex-1 sm:w-64">
                  ...
                  <input
                    ...
                    value={searchQuery}
                    onChange={(e) => setSearchQuery(e.target.value)}
                    className="w-full pl-9 pr-3 py-2 rounded-xl bg-white/[0.04] border border-white/10 text-white placeholder-slate-500 focus:outline-none focus:border-cyan-400"
                  />
                </div>
                ...
                <select
                  ...
                  className="px-3 py-2 rounded-xl bg-slate-900 border border-white/10 text-slate-300 focus:outline-none cursor-pointer"
                >
              ...
              <button
                ...
                className="w-full sm:w-auto px-4 py-2.5 rounded-xl bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 text-white font-bold flex items-center justify-center gap-2 cursor-pointer transition-all shadow-lg"
              >
            ...
            <div className="glass-panel rounded-3xl border border-white/10 overflow-hidden font-mono text-xs">
              <div className="overflow-x-auto">
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="border-b border-white/10 bg-white/[0.02] text-[10px] uppercase font-bold text-slate-400">
                      <th className="p-4">User Details</th>
                      <th className="p-4">Role</th>
                      <th className="p-4">Employee ID &amp; Dept</th>
                      <th className="p-4">Company Email Access</th>
                    ...
                    {appUsers.length === 0 ? (
                      <tr>
                        <td colSpan={6} className="p-8 text-center text-slate-500">
                          No user accounts loaded. Click "Create User Account" to add the first user.
                        </td>
                      </tr>
                    ) : (
                      appUsers
                        .filter((u) => {
                          ...
                          const matchesRole = statusFilter === 'all' || u.role?.toLowerCase() === statusFilter.toLowerCase();
                        ...
                        .map((u) => (
                          <tr key={u.id} className="hover:bg-white/[0.02] transition-colors">
                            <td className="p-4">
                              <div className="flex items-center gap-3">
                                <img src={u.image_url || '/assets/executive.png'} alt={u.name} className="w-9 h-9 rounded-xl object-cover border border-white/10" />
                                <div>
                            ...
                            <td className="p-4">
                              ...
                              >
                                {u.role === 'admin' ? '👑 Admin' : u.role === 'hr' ? '💼 HR' : '👤 Team Member'}
                              </span>
                            </td>
                            <td className="p-4">
                              <div className="font-bold text-white">{u.employee_id || 'ZNM-EMP'}</div>
                              <div className="text-[10px] text-slate-400">{u.designation} ({u.department})</div>
                            </td>
                            <td className="p-4">
                              <button
                                onClick={async () => {
                                  const newPerms = { ...u.permissions, email_access: !u.permissions?.email_access };
                                  try {
                                    await userManagementApi.update(u.id, { permissions: newPerms });
                                    addToast('Permissions Updated', `Email access set to ${newPerms.email_access ? 'YES' : 'NO'} for ${u.name}`, 'success');
                                    loadAppUsers();
                                  } catch (err) {
                                    addToast('Update Failed', 'Could not update permissions', 'error');
                                  }
                                }}
                                className={`px-2.5 py-1 rounded-xl border text-[10px] font-bold cursor-pointer transition-all ${
                                  u.permissions?.email_access
                                    ? 'bg-emerald-500/20 border-emerald-500/40 text-emerald-300'
                                    : 'bg-slate-500/10 border-slate-500/20 text-slate-400'
                                }`}
                                title="Toggle Email Access Permission"
                              >
                                {u.permissions?.email_access ? '✅ YES (Granted)' : '❌ NO (Denied)'}
                              </button>
                            </td>
                            <td className="p-4">
                              <button
                                onClick={async () => {
                                  try {
                                    const res = await userManagementApi.toggleStatus(u.id);
                                    addToast('Status Updated', res.data?.message || 'User status toggled', 'success');
                                    loadAppUsers();
                                  } catch (err) {
                                    addToast('Status Toggle Error', 'Failed to toggle status', 'error');
                                  }
                                }}
                            ...
                            <td className="p-4 text-right space-x-1.5">
                              ...
                              <button
                                onClick={() => {
                                  showConfirm(
                                    'Reset User Password',
                                    `Reset password for '${u.name}' to default 'Team@123'?`,
                                    async () => {
                                      try {
                                        const res = await userManagementApi.resetPassword(u.id);
                                        addToast('Password Reset', res.data?.message || 'Password reset to Team@123', 'success');
                                      } catch (err) {
                                        addToast('Reset Failed', 'Failed to reset password', 'error');
                                      }
                                    }
                                  );
                                }}
                              ...
                              <button
                                onClick={() => {
                                  showConfirm('Delete User Account', `Permanently delete user account '${u.name}'?`, async () => {
                                    try {
                                      await userManagementApi.delete(u.id);
                                      addToast('User Deleted', `Deleted ${u.name}`, 'warning');
                                      loadAppUsers();
                                    } catch (err) {
                                      addToast('Delete Failed', 'Failed to delete user', 'error');
                                    }
                                  });
                                }}
        ...
        {/* TAB 0-B: ADMIN NOTIFICATION DISPATCHER PANEL */}
        {activeTab === 'admin-notifs' && (
          <div className="space-y-8 font-mono text-xs">
            {/* Top Metrics & Dispatch Button */}
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
              <div className="modern-dashboard-card p-6 flex items-center justify-between">
                <div className="space-y-1">
                  <span className="text-[10px] text-slate-400 uppercase tracking-wider block">Dispatched Notifications</span>
                  <span className="text-3xl font-extrabold text-white block">{adminNotifications.length}</span>
                  <span className="text-[10px] text-cyan-400 block">Broadcasts &amp; Direct Alerts</span>
                </div>
                <div className="p-3.5 rounded-xl bg-cyan-500/10 text-cyan-400 border border-cyan-500/20">
              ...
              <div className="modern-dashboard-card p-6 flex items-center justify-between">
                <div className="space-y-1">
                  <span className="text-[10px] text-slate-400 uppercase tracking-wider block">Broadcast Notices</span>
                  <span className="text-3xl font-extrabold text-purple-300 block">
                    {adminNotifications.filter((n) => n.target_type === 'broadcast').length}
                  </span>
                  <span className="text-[10px] text-purple-400 block">Sent to All Members</span>
                </div>
                <div className="p-3.5 rounded-xl bg-purple-500/10 text-purple-400 border border-purple-500/20">
              ...
              <div className="modern-dashboard-card p-6 flex items-center justify-between">
                <div className="space-y-1">
                  <span className="text-[10px] text-slate-400 uppercase tracking-wider block">Dispatch New Alert</span>
                  <button
                    onClick={() => setIsNotifModalOpen(true)}
                    className="px-4 py-2 rounded-xl bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 text-white font-bold flex items-center gap-2 cursor-pointer transition-all shadow-lg mt-1"
                  >
                    <Plus className="w-4 h-4" /> Dispatch Notification
                  </button>
                </div>
              </div>
            </div>
                ...
                <table className="w-full text-left border-collapse">
                  ...
                  <tbody className="divide-y divide-white/5">
                    {adminNotifications.length === 0 ? (
                      <tr>
                        <td colSpan={6} className="p-8 text-center text-slate-500">
                          No notifications created yet. Click "Dispatch Notification" above.
                        </td>
                      </tr>
                    ) : (
                      adminNotifications.map((n) => (
                        <tr key={n.id} className="hover:bg-white/[0.02] transition-colors">
                          <td className="p-4 max-w-sm">
                            ...
                            <div className="text-[11px] text-slate-400 leading-relaxed line-clamp-2">{n.message}</div>
                          </td>
                          <td className="p-4">
                          ...
                          <td className="p-4">
                            <span className="px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase bg-purple-500/10 text-purple-300 border border-purple-500/30">
                              {n.target_type === 'broadcast' ? '📢 Broadcast (All)' : `👤 User (${n.target_user_id || 'Individual'})`}
                            </span>
                          </td>
                          <td className="p-4 text-slate-300">{n.created_by || 'Admin'}</td>
                          <td className="p-4 text-slate-400">{new Date(n.created_at).toLocaleString()}</td>
                          <td className="p-4 text-right">
                            <button
                              onClick={async () => {
                                showConfirm('Delete Notification', `Delete notification '${n.title}'?`, async () => {
                                  try {
                                    await adminNotificationApi.delete(n.id);
                                    addToast('Notification Deleted', 'Notification removed', 'warning');
                                    loadAdminNotifications();
                                  } catch (err) {
                                    addToast('Delete Failed', 'Could not delete notification', 'error');
                                  }
                                });
                              }}
                              className="p-1.5 rounded-lg bg-red-500/10 hover:bg-red-500/20 text-red-400 border border-red-500/20 cursor-pointer"
      ...
      {/* 12. CREATE / EDIT USER & RBAC MODAL */}
      <AnimatePresence>
        {isUserModalOpen && editingUser && (
          ...
          >
            <motion.div
              ...
              animate={{ scale: 1, opacity: 1 }}
              exit={{ scale: 0.95, opacity: 0 }}
              className="glass-panel p-6 sm:p-8 rounded-3xl border border-cyan-500/40 max-w-xl w-full my-8 space-y-6 max-h-[90vh] overflow-y-auto font-mono text-xs text-slate-100"
            >
              <div className="flex items-center justify-between border-b border-white/10 pb-4">
                <h3 className="text-lg font-bold font-display text-white flex items-center gap-2">
                  <UserCheck className="w-5 h-5 text-cyan-400" />
                  {editingUser.id ? 'Edit User Account & Permissions' : 'Create New Portal User Account'}
                </h3>
                <button onClick={() => setIsUserModalOpen(false)} className="p-1 text-slate-400 hover:text-white bg-white/5 rounded-lg cursor-pointer">
                  <X className="w-5 h-5" />
                </button>
              </div>
              ...
              <form
                onSubmit={async (e) => {
                  ...
                  try {
                    if (editingUser.id) {
                      await userManagementApi.update(editingUser.id, editingUser);
                      addToast('User Updated', `Updated account details for ${editingUser.name}`, 'success');
                    } else {
                      const res = await userManagementApi.create(editingUser);
                      addToast('User Created', res.data?.message || `Created account for ${editingUser.name}`, 'success');
                    }
                    setIsUserModalOpen(false);
                    loadAppUsers();
                  } catch (err: any) {
                    addToast('Operation Error', err.response?.data?.message || err.message || 'Failed to save user account', 'error');
                  }
                }}
                className="space-y-4 font-mono text-xs"
              >
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      type="text"
                      required
                      placeholder="e.g. Sangita Sahoo"
                      value={editingUser.name || ''}
                      onChange={(e) => setEditingUser({ ...editingUser, name: e.target.value })}
                      className="w-full px-3.5 py-2.5 rounded-xl bg-white/[0.04] border border-white/10 text-white placeholder-slate-500 focus:outline-none focus:border-cyan-400"
                    />
                  </div>
                ...
                {!editingUser.id && (
                  <div>
                    ...
                    <input
                      type="text"
                      placeholder="Leave empty for default: Team@123"
                      value={editingUser.password || ''}
                      onChange={(e) => setEditingUser({ ...editingUser, password: e.target.value })}
                      className="w-full px-3.5 py-2.5 rounded-xl bg-white/[0.04] border border-white/10 text-white placeholder-slate-500 focus:outline-none focus:border-cyan-400"
                    />
                    <span className="text-[10px] text-cyan-400">Default initial password assigned if left blank: Team@123</span>
                  </div>
                )}

                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-slate-300 font-bold mb-1">Portal Role *</label>
                    <select
                      value={editingUser.role || 'team_member'}
                      onChange={(e) => setEditingUser({ ...editingUser, role: e.target.value })}
                      className="w-full px-3.5 py-2.5 rounded-xl bg-slate-900 border border-white/10 text-white focus:outline-none focus:border-cyan-400 cursor-pointer"
                    >
                      <option value="team_member">Team Member (/team-login)</option>
                      <option value="hr">HR Personnel (/hr-login)</option>
                      <option value="admin">Administrator (/admin)</option>
                      <option value="manager">Manager (Future Ready)</option>
                ...
                {/* Company Email Access Permission Checkbox */}
                <div className="p-4 rounded-2xl bg-white/[0.03] border border-white/10 space-y-2">
                  <label className="flex items-center gap-3 cursor-pointer">
                    <input
                      type="checkbox"
                      checked={editingUser.permissions?.email_access === true}
                      onChange={(e) =>
                        setEditingUser({
                          ...editingUser,
                          permissions: { ...editingUser.permissions, email_access: e.target.checked },
                        })
                      }
                      className="w-4 h-4 rounded text-cyan-500 focus:ring-cyan-400 bg-slate-900 border-white/20"
                    />
                    <div>
                      <span className="font-bold text-white text-xs block">Grant Company Email Access Permission</span>
                      <span className="text-[10px] text-slate-400 block">
                        Allows user (e.g. HR) to use the company email system (Compose, Drafts, History, Templates).
                      </span>
                    </div>
                  </label>
                </div>
      ...
      {/* 13. DISPATCH NOTIFICATION MODAL */}
      <AnimatePresence>
        {isNotifModalOpen && (
          <motion.div
            initial={{ opacity: 0 }}
          ...
          >
            <motion.div
              ...
              animate={{ scale: 1, opacity: 1 }}
              exit={{ scale: 0.95, opacity: 0 }}
              className="glass-panel p-6 sm:p-8 rounded-3xl border border-purple-500/40 max-w-xl w-full my-8 space-y-6 max-h-[90vh] overflow-y-auto font-mono text-xs text-slate-100"
            >
              <div className="flex items-center justify-between border-b border-white/10 pb-4">
                <h3 className="text-lg font-bold font-display text-white flex items-center gap-2">
                  <Bell className="w-5 h-5 text-purple-400" />
                  Dispatch System Notification Alert
                </h3>
                <button onClick={() => setIsNotifModalOpen(false)} className="p-1 text-slate-400 hover:text-white bg-white/5 rounded-lg cursor-pointer">
                  <X className="w-5 h-5" />
                </button>
              </div>
                ...
                onSubmit={async (e) => {
                  ...
                  try {
                    const res = await adminNotificationApi.create({
                      ...
                      type: newNotifType,
                      target_type: newNotifTargetType,
                      target_user_id: newNotifTargetType === 'individual' ? newNotifTargetUser : null,
                    });

                    if (res.data && res.data.success) {
                      addToast('Notification Sent', res.data.message, 'success');
                      setNewNotifTitle('');
                      setNewNotifMessage('');
                    ...
                    addToast('Dispatch Error', err.response?.data?.message || 'Failed to dispatch notification', 'error');
                  }
                }}
                className="space-y-4 font-mono text-xs"
              >
                <div>
                  ...
                  <input
                    type="text"
                    required
                    placeholder="e.g. 📢 All-Hands Project Meeting Today at 4 PM"
                    value={newNotifTitle}
                    onChange={(e) => setNewNotifTitle(e.target.value)}
                    className="w-full px-3.5 py-2.5 rounded-xl bg-white/[0.04] border border-white/10 text-white placeholder-slate-500 focus:outline-none focus:border-purple-400 font-bold"
                  />
                </div>

                <div>
                  <label className="block text-slate-300 font-bold mb-1">Notification Body Message *</label>
                  <textarea
                    rows={4}
                    required
                    placeholder="Enter full details of the notification announcement..."
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-slate-300 font-bold mb-1">Notification Category / Type</label>
                    <select
                      value={newNotifType}
                      onChange={(e) => setNewNotifType(e.target.value)}
                      className="w-full px-3.5 py-2.5 rounded-xl bg-slate-900 border border-white/10 text-white focus:outline-none focus:border-purple-400 cursor-pointer"
                    >
                      <option value="info">ℹ️ Information</option>
                      <option value="meeting">📅 Meeting Invite</option>
                      <option value="payment">💳 Payment / Stipend Released</option>
                      <option value="project">💼 Project Update</option>
                      <option value="success">✅ Success Announcement</option>
                      <option value="warning">⚠️ Warning / Deadline Changed</option>
                  ...
                  <div>
                    ...
                    <select
                      value={newNotifTargetType}
                      onChange={(e) => setNewNotifTargetType(e.target.value as any)}
                      className="w-full px-3.5 py-2.5 rounded-xl bg-slate-900 border border-white/10 text-white focus:outline-none focus:border-purple-400 cursor-pointer"
                    >
                      <option value="broadcast">📢 Broadcast to All Portal Members</option>
                      <option value="individual">👤 Send to Individual User</option>
                    </select>
                  </div>
                </div>

                {newNotifTargetType === 'individual' && (
                  <div>
                    <label className="block text-slate-300 font-bold mb-1">Select Target User *</label>
                    <select
                ...
                <div className="flex items-center justify-end gap-3 pt-4 border-t border-white/10">
                  ...
                  >
                    <Send className="w-4 h-4" /> Dispatch Alert Now
                  </button>
                </div>
              </form>
            </motion.div>
