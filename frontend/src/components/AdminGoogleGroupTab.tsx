...
export const AdminGoogleGroupTab: React.FC<AdminGoogleGroupTabProps> = ({ showToast, showConfirm }) => {
  ...
  const handleTriggerSync = async () => {
    ...
    try {
      ...
      if (data?.success) {
        ...
        if (showToast) {
          showToast('Sync Successful', `Added ${data.addedCount || 0} new member(s) to Google Group.`, 'success');
        }
  ...
  const handleRemoveMember = (memberEmail: string) => {
    const confirmAction = async () => {
      try {
        ...
        if (res.data?.success) {
          if (showToast) {
            showToast('Member Removed', `${memberEmail} has been removed from the Google Group.`, 'success');
          }
    ...
    if (showConfirm) {
      showConfirm(
        ...
        `Are you sure you want to remove "${memberEmail}" from zenemoocommunity@googlegroups.com? Their community record in Supabase will remain intact.`,
        confirmAction,
        {
      ...
      if (window.confirm(`Are you sure you want to remove ${memberEmail} from the Google Group?`)) {
        confirmAction();
      }
    }
  };
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-3">
            <button
              ...
              disabled={isLoadingOverview || isLoadingMembers}
            ...
            >
              <RefreshCw className={`h-4 w-4 ${isLoadingOverview || isLoadingMembers ? 'animate-spin' : ''}`} />
              Refresh
            </button>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div className="rounded-2xl border border-gray-800 bg-gray-900/60 p-5 shadow-lg backdrop-blur-md">
          <div className="flex items-center justify-between">
            <span className="text-xs font-medium uppercase tracking-wider text-gray-400">Total Supabase Eligible</span>
          ...
          <div className="mt-3 flex items-baseline gap-2">
            ...
            <span className="text-xs text-gray-400">community emails</span>
          </div>
          <p className="mt-2 text-xs text-gray-400">Aggregated across 8 Supabase data sources</p>
        </div>

        {/* Card 2: Current Group Members */}
        <div className="rounded-2xl border border-gray-800 bg-gray-900/60 p-5 shadow-lg backdrop-blur-md">
          <div className="flex items-center justify-between">
            <span className="text-xs font-medium uppercase tracking-wider text-gray-400">Google Group Members</span>
          ...
          <p className="mt-2 text-xs text-gray-400">Live count via Admin Directory API</p>
        </div>
          ...
          <div className="mt-3 flex items-baseline gap-2">
            ...
            <span className="text-xs text-purple-400">
              {overview?.totalEligible
                ? `${Math.round(((overview.syncedCount || 0) / overview.totalEligible) * 100)}% coverage`
                : '0%'}
            </span>
          </div>
          <p className="mt-2 text-xs text-gray-400">Supabase users present in Group</p>
        </div>
        ...
        <div className="rounded-2xl border border-gray-800 bg-gray-900/60 p-5 shadow-lg backdrop-blur-md">
          ...
          <p className="mt-2 text-xs text-gray-400">Ready to be added on next sync run</p>
        </div>
      </div>
      ...
      {/* 4. Member Management Table Section */}
                  ...
                  <tr key={m.email} className="transition-colors hover:bg-gray-800/30">
                    ...
                    <td className="px-5 py-3 text-right">
                      <button
                        ...
                        title="Remove member from Google Group"
                      >
