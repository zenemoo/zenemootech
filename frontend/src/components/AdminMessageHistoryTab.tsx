...
export const AdminMessageHistoryTab: React.FC<AdminMessageHistoryTabProps> = ({
  ...
  const fetchLogs = useCallback(
    async (pageToFetch: number, sizeToFetch: number, statusToFetch: string, searchToFetch: string, rangeToFetch: string) => {
      setIsLoading(true);
      try {
        ...
        showToast('Failed to load email history', err.message || 'Network error', 'error');
      } finally {
        setIsLoading(false);
  ...
  const handleRefresh = async () => {
    ...
    await fetchLogs(currentPage, pageSize, statusFilter, appliedSearch, dateRangeFilter);
    showToast('Email history refreshed from Supabase', undefined, 'success');
  };
    ...
    <div className="space-y-6 font-sans">
      ...
      <div className="space-y-4">
        {isLoading ? (
          ...
          <div className="glass-panel p-8 rounded-2xl border border-white/10 space-y-4 text-center">
            <div className="flex items-center justify-center gap-3 py-12">
              <RefreshCw className="w-6 h-6 text-cyan-400 animate-spin" />
              <span className="text-sm font-mono text-slate-300 font-semibold">
                Loading encrypted email logs from Supabase...
              </span>
            </div>
          </div>
        ) : logs.length === 0 ? (
