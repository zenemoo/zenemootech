...
export const AdminTalentNetworkTab: React.FC = () => {
  ...
  return (
    <div className="space-y-6 font-sans max-w-7xl mx-auto pb-12">
      ...
      <div className="glass-panel p-6 sm:p-8 rounded-3xl border border-cyan-500/30 flex flex-col md:flex-row items-start md:items-center justify-between gap-6 shadow-2xl relative overflow-hidden">
        ...
        <div className="flex flex-wrap items-center gap-2.5 w-full md:w-auto">
          <button
            ...
            className={`px-4 py-2.5 rounded-2xl font-bold text-xs font-mono flex items-center justify-center gap-2 cursor-pointer transition-all ${
              activeAdminTab === 'roster' && !showAnalytics
          ...
          <button
            onClick={() => {
              setActiveAdminTab('languages');
              setShowAnalytics(false);
            }}
            className={`px-4 py-2.5 rounded-2xl font-bold text-xs font-mono flex items-center justify-center gap-2 cursor-pointer transition-all ${
              activeAdminTab === 'languages' && !showAnalytics
                ? 'bg-purple-500 text-white shadow-lg shadow-purple-500/20'
                : 'bg-white/5 hover:bg-white/10 text-slate-300 border border-white/10'
            }`}
          >
            <Globe className="w-4 h-4 text-purple-400" /> Language Management
          </button>

          <button
            onClick={() => {
              if (showAnalytics || activeAdminTab === 'analytics') {
                setShowAnalytics(false);
                setActiveAdminTab('roster');
              } else {
            ...
            <div className="flex items-center justify-between border-b border-white/10 pb-4">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="px-3.5 py-1.5 rounded-xl bg-cyan-500 hover:bg-cyan-400 text-black font-extrabold flex items-center gap-1.5 shadow-lg shadow-cyan-500/20 cursor-pointer"
                  title="Edit complete profile details"
                >
                  <Edit3 className="w-3.5 h-3.5 text-black" /> Edit Profile
                </button>
                <button
      ...
      {/* ── 8. ADMIN CANDIDATE PROFILE EDIT MODAL ── */}
      {editingCandidate && (
