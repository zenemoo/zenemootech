...
export const EnterpriseTeamDirectory: React.FC<EnterpriseTeamDirectoryProps> = ({
  ...
  const filteredMembers = members.filter((m) => {
    ...
    const matchesSearch =
      ...
      (m.personal_phone && m.personal_phone.toLowerCase().includes(q)) ||
      (m.personal_email && m.personal_email.toLowerCase().includes(q)) ||
      (m.upi_id && m.upi_id.toLowerCase().includes(q)) ||
  ...
  return (
    <div className="space-y-6 font-mono text-xs w-full max-w-full overflow-x-hidden">
      ...
      <div className="glass-panel p-5 sm:p-7 rounded-3xl border border-cyan-500/30 shadow-2xl space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          ...
          <div className="sm:col-span-2 relative">
            ...
            <input
              type="text"
              placeholder="Search by Name, Employee ID (ZNM-30A53), Phone, Email, UPI, Skills..."
              value={searchQuery}
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="flex items-center gap-3">
                    <img
                      src={member.photo}
                      alt={member.name}
                      className="w-14 h-14 rounded-2xl object-cover border border-cyan-400/60 group-hover:scale-105 transition-transform"
                    />
                    <div className="truncate">
                      <h3 className="font-bold text-white text-sm truncate group-hover:text-cyan-300 transition-colors">
                        {member.name}
                      </h3>
                      <div className="text-xs text-cyan-400 font-bold font-mono truncate">{member.employee_id}</div>
                      <div className="text-[10px] text-slate-400 truncate">{member.designation}</div>
                    </div>
                  </div>
                </div>
              ...
              <div className="pt-3 border-t border-white/10 flex items-center justify-between text-[10px]">
                {member.is_private_profile_completed ? (
                  <span className="text-emerald-400 font-bold flex items-center gap-1">
                    <CheckCircle className="w-3 h-3 text-emerald-400" /> Private Profile Ready
                  </span>
                ) : (
      ...
      {/* 4. ROLE-BASED EMPLOYEE DETAIL MODAL / DRAWER */}
      {selectedMember && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-3 sm:p-6 overflow-y-auto">
          <div className="w-full max-w-3xl bg-[#090d16] border border-cyan-500/40 rounded-3xl p-5 sm:p-7 space-y-6 shadow-2xl font-mono text-xs max-h-[92vh] overflow-y-auto">
            ...
            <div className="flex items-start justify-between border-b border-white/10 pb-4">
              <div className="flex items-center gap-4">
                ...
                <div className="space-y-1">
                  <h3 className="text-base sm:text-xl font-bold font-display text-white">{selectedMember.name}</h3>
                  <div className="flex items-center gap-2 flex-wrap text-xs">
                    <span className="text-cyan-400 font-bold">{selectedMember.employee_id}</span>
                    <span className="text-slate-500">&bull;</span>
                    <span className="text-slate-300">{selectedMember.designation}</span>
                    <span className="text-slate-500">&bull;</span>
            ...
            {/* Quick Contact Action Buttons (Admin & HR) */}
            {(userRole === 'admin' || userRole === 'hr') && (
              <div className="p-4 rounded-2xl bg-cyan-500/10 border border-cyan-500/30 flex flex-wrap items-center justify-between gap-3">
                <span className="font-bold text-cyan-300 text-xs">Direct Operational Actions:</span>
                <div className="flex flex-wrap items-center gap-2">
                  {selectedMember.personal_phone && (
                    <a
                      href={`tel:${selectedMember.personal_phone}`}
                      className="px-3 py-1.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-black font-bold flex items-center gap-1.5 transition-all text-xs"
                    >
                      <Phone className="w-3.5 h-3.5 text-black" /> Call
                    </a>
                  )}

                  {selectedMember.company_email && (
                    <a
                      href={`mailto:${selectedMember.company_email}`}
                      className="px-3 py-1.5 rounded-xl bg-cyan-600 hover:bg-cyan-500 text-black font-bold flex items-center gap-1.5 transition-all text-xs"
                    >
                  ...
                  {selectedMember.personal_phone && (
                    <a
                      href={`https://wa.me/${selectedMember.personal_phone.replace(/[^0-9]/g, '')}`}
                      target="_blank"
                      rel="noreferrer"
                      className="px-3 py-1.5 rounded-xl bg-emerald-500/20 hover:bg-emerald-500/30 text-emerald-300 border border-emerald-500/40 font-bold flex items-center gap-1.5 text-xs"
                    >
                  ...
                  {selectedMember.personal_phone && (
                    <button
                      onClick={() => copyToClipboard(selectedMember.personal_phone!, 'Phone Number')}
                      className="px-3 py-1.5 rounded-xl bg-white/[0.04] hover:bg-white/[0.08] border border-white/10 text-slate-300 flex items-center gap-1 text-xs cursor-pointer"
                    >
            ...
            {userRole !== 'team' && activeModalTab === 'personal' && (
              <div className="space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="p-3.5 rounded-2xl bg-white/[0.03] border border-white/5 space-y-1">
                    <div className="text-[10px] text-slate-400 uppercase">Personal Phone Number</div>
                    <div className="text-sm font-bold text-white">{selectedMember.personal_phone || 'Not Provided'}</div>
                  </div>

                  <div className="p-3.5 rounded-2xl bg-white/[0.03] border border-white/5 space-y-1">
                    <div className="text-[10px] text-slate-400 uppercase">Personal Email</div>
                    <div className="text-sm font-bold text-white">{selectedMember.personal_email || 'Not Provided'}</div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <div className="p-3 rounded-2xl bg-white/[0.03] border border-white/5 space-y-1">
                    ...
                    <div className="text-xs font-bold text-white">{selectedMember.dob || 'Not Specified'}</div>
                  </div>

                  <div className="p-3 rounded-2xl bg-white/[0.03] border border-white/5 space-y-1">
                    <div className="text-[10px] text-slate-400 uppercase">Blood Group</div>
                    <div className="text-xs font-bold text-red-400">{selectedMember.blood_group || 'Not Specified'}</div>
                  </div>
            ...
            {userRole !== 'team' && activeModalTab === 'address' && (
              <div className="space-y-4">
                <div className="p-4 rounded-2xl bg-white/[0.03] border border-white/5 space-y-1">
                  ...
                  <div className="text-xs text-white leading-relaxed">{selectedMember.address_current || 'No current address submitted.'}</div>
                </div>

                <div className="p-4 rounded-2xl bg-white/[0.03] border border-white/5 space-y-1">
                  <div className="text-[10px] text-slate-400 uppercase">Permanent Address</div>
                  <div className="text-xs text-white leading-relaxed">{selectedMember.address_permanent || 'No permanent address submitted.'}</div>
                </div>
              </div>
            )}
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="p-3.5 rounded-2xl bg-white/[0.03] border border-white/5 space-y-1">
                    ...
                    <div className="text-sm font-bold text-white">{selectedMember.bank_name || 'Bank Not Specified'}</div>
                    <div className="text-xs text-cyan-300">{selectedMember.account_holder}</div>
                  </div>

                  <div className="p-3.5 rounded-2xl bg-white/[0.03] border border-white/5 space-y-1">
                    <div className="text-[10px] text-slate-400 uppercase">Bank Account Number</div>
                    <div className="text-sm font-bold text-emerald-300">{selectedMember.bank_account_number || 'Not Provided'}</div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <div className="p-3 rounded-2xl bg-white/[0.03] border border-white/5 space-y-1">
                    ...
                    <div className="text-xs font-bold text-white">{selectedMember.ifsc_code || 'Not Provided'}</div>
                  </div>

                  <div className="p-3 rounded-2xl bg-white/[0.03] border border-white/5 space-y-1">
                    <div className="text-[10px] text-slate-400 uppercase">PAN Number</div>
                    <div className="text-xs font-bold text-white">{selectedMember.pan_number || 'Not Provided'}</div>
                  </div>

                  <div className="p-3 rounded-2xl bg-white/[0.03] border border-white/5 space-y-1">
                    <div className="text-[10px] text-slate-400 uppercase">UPI Virtual Handle</div>
                    <div className="text-xs font-bold text-purple-300">{selectedMember.upi_id || 'Not Provided'}</div>
                  </div>
                </div>
              </div>
            )}
              ...
              <div className="p-5 rounded-2xl bg-red-500/10 border border-red-500/30 space-y-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-2">
                  <div className="space-y-1">
                    ...
                    <div className="text-xs font-bold text-white">{selectedMember.emergency_contact_person || 'Not Provided'}</div>
                  </div>

                  <div className="space-y-1">
                    <div className="text-[10px] text-slate-400">Relationship</div>
                    <div className="text-xs font-bold text-white">{selectedMember.emergency_relationship || 'Not Specified'}</div>
                  </div>

                  <div className="space-y-1">
                    <div className="text-[10px] text-slate-400">Emergency Phone Number</div>
                    <div className="text-xs font-bold text-emerald-300">{selectedMember.emergency_contact_phone || 'Not Provided'}</div>
                  </div>
                </div>
              </div>
            )}
