          ...
          <div className="bg-[#090a0f] border border-white/10 rounded-3xl w-full max-w-2xl p-6 space-y-4 shadow-2xl relative">
            <div className="flex items-center justify-between gap-3 border-b border-white/10 pb-3">
              <div className="flex items-center gap-2.5 min-w-0 flex-1 pr-2">
                <span className="px-2.5 py-0.5 rounded-full text-[10px] font-mono bg-cyan-500/10 text-cyan-400 border border-cyan-500/30 shrink-0">
                  {previewFile.file_type}
                </span>
                <h3 className="text-xs sm:text-base font-bold text-white font-display truncate min-w-0 flex-1" title={previewFile.file_name}>
                  {previewFile.file_name}
                </h3>
              </div>
              <button
                onClick={() => setPreviewFile(null)}
                className="p-1.5 rounded-xl bg-slate-900 hover:bg-slate-800 text-slate-400 hover:text-white border border-slate-800 transition-colors shrink-0 cursor-pointer flex items-center justify-center"
                title="Close preview"
              >
