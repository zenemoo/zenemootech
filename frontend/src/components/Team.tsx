...
import { Users, Star, UserCheck, Mail, ArrowRight, X, Search, Sparkles } from 'lucide-react';
...
export const Team: React.FC = () => {
  ...
  // Screen size detection: 4 for mobile/tablet (< 1024px), 8 for desktop (>= 1024px)
  ...
  // Limit count: 4 on mobile/small screen, 8 on desktop
  const limitCount = isMobile ? 4 : 8;
  const displayedMembers = members.slice(0, limitCount);

  // Modal filtered members
  const modalMembers = members.filter((m) => {
    const query = searchQuery.toLowerCase();
    const matchesSearch =
      m.name.toLowerCase().includes(query) ||
      (m.designation || m.role || '').toLowerCase().includes(query) ||
    ...
    const matchesDept =
      selectedDepartment === 'All'
        ? true
        : dept.includes(selectedDepartment.toLowerCase());
  ...
  return (
    <section id="team" className="py-24 relative z-10 bg-noise">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {loading ? (
          ...
          <>
            ...
            {/* View Complete Team Directory Button */}
            {members.length > displayedMembers.length && (
              <div className="mt-12 text-center">
                <button
                  onClick={() => setShowFullModal(true)}
                  className="inline-flex items-center gap-2.5 px-8 py-4 rounded-2xl bg-gradient-to-r from-cyan-500 via-blue-600 to-purple-600 hover:from-cyan-400 hover:to-purple-500 text-white font-bold font-display text-sm tracking-wide shadow-xl shadow-cyan-500/25 hover:scale-[1.02] transition-all cursor-pointer"
                >
                  <Users className="w-5 h-5" />
                  View Complete Team Directory ({members.length} Members)
      ...
      {/* FULL TEAM DIRECTORY MODAL */}
      {showFullModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 bg-black/85 backdrop-blur-2xl">
          <div className="glass-panel w-full max-w-6xl max-h-[92vh] rounded-3xl border border-white/20 flex flex-col overflow-hidden shadow-2xl bg-[#07080f]">
            {/* Modal Header */}
            <div className="p-6 border-b border-white/10 bg-white/[0.02] flex items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-xl font-bold font-display text-white">Zenemoo Complete Team Directory</h3>
                  <p className="text-xs font-mono text-cyan-400">All {members.length} Active Specialists, Transcribers &amp; Leadership</p>
                </div>
              </div>
