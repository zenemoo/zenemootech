...
export const TalentHubLoginPage: React.FC<TalentHubLoginPageProps> = ({
  ...
  // ── State 1: Checking authentication, restoring session, or verifying registration status ──
  if (isLoading || isProfileLoading || isSigningIn || (session && isRegistered === null)) {
    return (
            ...
            <div className="flex items-center justify-center gap-2">
              ...
              <h3 className="text-sm font-semibold text-white">
                {isSigningIn
                  ...
                  : 'Verifying Contributor Account...'}
              </h3>
            </div>
            <p className="text-xs text-slate-400 mt-1">
              {isSigningIn
                ? 'Connecting your Zenemoo Talent Hub session'
                : session
                ? 'Checking your contributor profile'
                : 'Checking your Zenemoo Talent Hub registration'}
            </p>
          </div>
        </div>
      </div>
