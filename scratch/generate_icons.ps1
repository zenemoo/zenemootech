...
$sourcePath = "branding\Zenemoo_app_logo.png"
if (-not (Test-Path $sourcePath)) {
    Write-Error "Source image branding\Zenemoo_app_logo.png not found!"
    exit 1
}
...
function Resize-SplashImage {
    ...
    # Logo takes up ~40% of shorter dimension
...
# Generate standard Mipmap Launcher icons & Adaptive Foreground icons
$mipmapConfigs = @(
...
# Generate Notification icons (Monochrome alpha icon for Android status bar notifications)
$notificationConfigs = @(
...
# Generate Splash Screen images for all densities
$splashConfigs = @(
...
$srcImg.Dispose()
Write-Host "All Android icons and splash screens generated successfully!"
